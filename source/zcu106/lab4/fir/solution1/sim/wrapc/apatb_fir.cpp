...
static bool RTLOutputCheckAndReplacement(std::string &AESL_token, std::string PortName) {
  ...
  return err;}
extern "C" void fir_hw_stub_wrapper(volatile void *, short);

extern "C" void apatb_fir_hw(volatile void * __xlx_apatb_param_y, short __xlx_apatb_param_x) {
    ...
    string AESL_num;
{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          ...
          if (i > 0) {((char*)__xlx_apatb_param_y)[0*2+0] = y_pc_buffer[0].range(7, 0).to_int64();
((char*)__xlx_apatb_param_y)[0*2+1] = y_pc_buffer[0].range(15, 8).to_int64();
}
        } // end transaction
      } // end file is good
